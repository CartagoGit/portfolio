...
export class HeroMonitorFacade {
  ...
  readonly selectedPanel = computed(() => this.panels.find(({ id }) => id === this.activePanel()));
  ...
  selectPanel(panel: HeroPanelId): void {
    ...
    this.panelTransition.set(nextHeroPanelTransition(this.panelTransition()));
  ...
  gradient(value: number): string {
    ...
    if (this.palette() === 'lime') return 'linear-gradient(to top, #256c5d, #d8ff78)';
